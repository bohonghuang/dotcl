...
(defun bnd-member-p (sym bnd)
  ...
  distinguish same-name vars from different packages (e.g. PARSONIC vs
  BINSTRUCT both binding |(SIMPLE-ARRAY (UNSIGNED-BYTE 8) (*))|).
...
(defun lookup-local (sym)
  ...
  packages have a symbol with the same printed name (e.g. PARSONIC vs
  BINSTRUCT both binding |(SIMPLE-ARRAY (UNSIGNED-BYTE 8) (*))|)."
...
(defun local-bound-p (sym)
  ...
  This prevents a PARSONIC reference from matching a BINSTRUCT binding with
  the same printed name, which would suppress free-var capture and cause
